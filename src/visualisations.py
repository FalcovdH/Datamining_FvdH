...
class DataAnalyzer:
    ...
    def category_weekday_polar_plot(self):
        ...
        if self.df is not None:
            ...
            weekday_grouped_msg = (
                self.df.groupby(["year", "weekday"])["message"]
                .value_counts()
            ...
            weekday_grouped_msg["weekday"] = pd.Categorical(
                weekday_grouped_msg["weekday"], categories=weekday_order, ordered=True
            )

            # Sort the DataFrame by 'year' and 'weekday'
            weekday_grouped_msg = weekday_grouped_msg.sort_values(
                by=["year", "weekday"]
            )
            category_orders = {
                "weekday": [
    ...
    def categorie_common_words(self):
        ...
        if self.df is not None:
            ...
            custom_palette = [
                "red" if count < 80 else "green" for count in words_df["count"]
            ]
        ...
        if self.df is not None:
            ...
            if interval == "month":
                self.df["time_interval"] = (
                    self.df["timestamp"].dt.to_period("M").astype(str)
                )
            elif interval == "year":
                self.df["time_interval"] = (
                    self.df["timestamp"].dt.to_period("Y").astype(str)
                )
            else:
                raise ValueError(
                    "Invalid interval. Supported values are 'month' and 'year'."
                )
            ...
            for category, pattern in word_patterns.items():
                self.df[f"{category}_mentions"] = self.df["message"].apply(
                    lambda x: len(pattern.findall(x))
                )

            # Group by time_interval and calculate the total number of mentions for each word category
            mentions_over_time = (
                self.df.groupby("time_interval")[
                    [f"{category}_mentions" for category in word_patterns]
                ]
                .sum()
                .reset_index()
            )
            ...
            for category in word_patterns.keys():
                sns.lineplot(
                    data=mentions_over_time,
                    x="time_interval",
                    y=f"{category}_mentions",
                    label=category,
            ...
            # Customize the plot
            plt.title(
                f"Winst/verlies vermeldingen (Gegroepeerd bij {interval.capitalize()})"
            )
            plt.xlabel("Jaren")
            ...
            plt.annotate(
                "Tijd voor een transfer",
                xy=(0.80, 0.95),
                xytext=(0.65, 0.95),
                xycoords="axes fraction",
                textcoords="axes fraction",
                arrowprops=dict(
                    facecolor="black", arrowstyle="wedge,tail_width=0.7", lw=1
                ),
                fontsize=11,
                ha="center",
                va="center",
    ...
    def time_bbq_mentions(self):
        ...
        if self.df is not None:
            ...
            bbq_pattern = re.compile(
                r"\b(?:BBQ|Bbq|bbq|barbecue|barbeque|bbq\'en|bbq\'ing)\b", re.IGNORECASE
            )

            # Count occurrences of "BBQ" or "bbq" in each message
            self.df["bbq_mentions"] = self.df["message"].apply(
                lambda x: len(bbq_pattern.findall(x))
            )

            # Group by month and calculate the average number of BBQ mentions
            bbq_mentions_over_time = (
                self.df.groupby("month")["bbq_mentions"].mean().reset_index()
            )
            ...
            # Customize the plot
            plt.title(
                "Gemiddelde aantal vermeldingen BBQ in de tekst (Grouped by Month)"
            )
            plt.xlabel("Maand")
            plt.ylabel("Gemiddeld aantal vermeldingen BBQ")

            plt.annotate(
                "Wanneer heeft de BBQ plaatsgevonden?? \n \n Antwoord: Nog nooit!",
                xy=(6, 4.5),
                xytext=(0.35, 0.9),
                xycoords="axes fraction",
                textcoords="axes fraction",
    ...
    def distributie_plot_message_length(self):
        ...
        if self.df is not None:
            ...
            df_filtered = self.df[
                (self.df["year"] != 2017) & (self.df["year"] != 2024)
            ].copy()  # Use copy() to create a copy
            ...
            g_day.map(
                sns.histplot, "message_length", color="green", kde=True
            )  # Use 'message_length' instead of x="message_length"
            g_day.set(
                xticks=range(0, 300, 50), xlabel="Message Length", ylabel="Count"
            ...
            g_quarter = sns.FacetGrid(
                df_filtered,
                col="quarter",
                col_wrap=4,
                height=3,
                col_order=quarters_order,
            )
            g_quarter.map(
                sns.histplot, "message_length", color="blue", kde=True
            )  # Use 'message_length' instead of x="message_length"
            g_quarter.set(
                xticks=range(0, 300, 50), xlabel="Message Length", ylabel="Count"
            )  # Set x-axis values and labels
            ...
            g_year.map(
                sns.histplot, "message_length", color="orange", kde=True
            )  # Use 'message_length' instead of x="message_length"
            g_year.set(
                xticks=range(0, 300, 50), xlabel="Message Length", ylabel="Count"
            )  # Set x-axis values and labels
            ...
            g_day.savefig(
                self.save_location + "distributie_message_length_per_day_of_week.png"
            )
            g_quarter.savefig(
                self.save_location + "distributie_message_length_per_quarter.png"
            )
            g_year.savefig(
                self.save_location + "distributie_message_length_per_year.png"
            )
    ...
    def relations_link_vs_media(self):
        ...
        if self.df is not None:
            ...
            self.df["has_media"] = self.df["message"].str.contains(
                r"<Media weggelaten>", case=False, regex=True
            )
            ...
            ax = sns.scatterplot(
                ...
                alpha=0.3,
                hue="author_category",
                palette={"Teamleider": "red", "Teamlid": "blue", "Ex-teamlid": "gray"},
            )
            ...
            plt.title(
                "Is er een rol voor de ex-teamlid Stephan als teamleider?", fontsize=9
            )
            plt.suptitle(
                "De teamleiders zijn actief gebruikers van media en links!", fontsize=12
            )
            ...
            for i, (author, sum_has_media) in enumerate(top_3_has_media.items()):
                plt.text(
                    p.loc[author, "has_link"],
                    p.loc[author, "has_media"],
                    f"{author}",
                    fontsize=8,
                    fontweight="bold",
                )
...
def main_v():
    # Define paths
    datafile_path = r"C:/Users/a427617/Documents/Master Data science/Blok 3 - Data mining/Data-Mining---2024/data/processed/Zaalvoetbal.parq"
